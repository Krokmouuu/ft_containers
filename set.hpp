...
namespace ft 
{
    ...
    public:
        ...
        set(InputIt first, InputIt last, const Compare& comp = Compare(), const allocator_type& alloc = allocator_type(), typename ft::enable_if<!ft::is_integral<InputIt>::value, InputIt>::type* = nullptr): _set_t(comp, alloc)   
        { 
                insert(first, last); 
        }
        ...
        { 
            return _set_t.max_size();
        }

        allocator_type get_allocator() const 
        { 
            return _set_t.get_allocator(); 
        }
        ...
        void insert(InputIt start, InputIt end, typename ft::enable_if<!ft::is_integral<InputIt>::value, InputIt>::type* = NULL)
        {
        ...
        void swap(set& x) 
        {
            _set_t.swap(x._set_t);
        }

        size_type count(const key_type& x) const 
        { 
            return _set_t.find(x) == _set_t.end() ? 0 : 1; 
        }

        iterator find(const key_type& x) 
        { 
            return _set_t.find(x);
        }
        ...
        { 
            return _set_t.key_comp();
        }

        value_compare value_comp() const 
        { 
            return value_compare(_set_t.key_comp()); 
        }

        template <typename _KeyT, typename _CompT, typename _AllocT>
        friend bool operator==(const set<_KeyT, _CompT, _AllocT>&, const set<_KeyT, _CompT, _AllocT>&);
        template <typename _KeyT, typename _CompT, typename _AllocT>
        friend bool operator<(const set<_KeyT, _CompT, _AllocT>&, const set<_KeyT, _CompT, _AllocT>&);
    ...
    template <typename _KeyT, typename _CompT, typename _AllocT>
    inline bool operator==(const ft::set<_KeyT, _CompT, _AllocT>& lhs, const ft::set<_KeyT, _CompT, _AllocT>& rhs)
    {
    ...
    {
        template <typename _KeyT, typename _CompT, typename _AllocT>
        void swap(ft::set<_KeyT, _CompT, _AllocT>& lhs, ft::set<_KeyT, _CompT, _AllocT>& rhs) 
        {
            lhs.swap(rhs);
        }
    };